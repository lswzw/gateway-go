...
为了保持插件文档的一致性和可读性，每个插件都应该包含以下标准化的文档结构：
...
## 十、插件配置
在路由或全局plugins中添加`plugin_name`插件即可。
```
...
1. **概述部分**：简洁明了地描述插件功能
2. **设计目标**：列出3-5个核心设计目标
3. **流程图**：用文字描述处理流程，确保逻辑清晰
4. **配置参数**：使用表格格式，包含所有可配置参数
5. **配置示例**：提供完整的YAML配置示例
6. **运行属性**：明确插件执行阶段和优先级
7. **请求示例**：提供实际的curl命令示例
8. **处理流程**：按步骤列出处理逻辑
9. **错误码**：列出所有可能的错误码和说明
10. **插件配置**：说明如何启用插件
...
每个插件的文档应放在对应的插件目录下：
```
internal/plugin/plugins/yourplugin/README.md
```
...
所有内置插件的说明文档已按统一标准（参考灰度发布插件示例）重写，详见 reference.md 文件。请查阅 reference.md 获取各插件的详细用法、配置、流程、错误码等信息。 